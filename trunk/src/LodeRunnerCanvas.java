...
class LodeRunnerCanvas extends GameCanvas
{
  ...
  public void paint(Graphics g)
  {
    ...
    if (stage != null)
    {
      ...
      if (isPaused || !stage.isLoaded)
      {
        ...
        dg.setARGBColor(0xa0000000); g.fillRect(0, 0, w0, h0);
        ...
        g.drawString("Level " + Integer.toString(level % LodeRunnerStage.GAME_LEVELS + 1), x + LodeRunnerStage.SPRITE_WIDTH[LodeRunnerStage.SPRITE_NORMAL] + 1, y + cy + 3, Graphics.TOP | Graphics.LEFT);
        stage.sprites[LodeRunnerStage.SPRITE_NORMAL].paint(g, stage.spriteMap[LodeRunnerStage.TILE_HERO], x + 3 * cx / 4, y + cy + 3);
        g.drawString("x" + Integer.toString(lifes), x + 3 * cx / 4 + LodeRunnerStage.SPRITE_WIDTH[LodeRunnerStage.SPRITE_NORMAL] + 1, y + cy + 3, Graphics.TOP | Graphics.LEFT);
  ...
  public void stageOver(boolean hasCompleted)
  {
    ...
    if (hasCompleted)
    {
      level++; if (level == LodeRunnerStage.MAX_LEVELS) level = 0;
    ...
    else
    {
      ...
      if (lifes < 0)
      {
        lifes = MAX_LIFES; level = 0; pauseMessage = "Game Over";
      }
      else pauseMessage = "Try again...";
    }
