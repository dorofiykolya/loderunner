...
abstract class LodeRunnerCharacter
{
  ...
  public void moveToTile(int tileIndex)
  {
    ...
    lookLeft = false; xAdjust = yAdjust = 0; xDelta = yDelta = delayBusy = 0;
    currentMove = MOVE_NONE; nextMove = MOVE_NONE;
  }
  ...
  protected int getFrame(int[] keyFrames)
  {
    ...
    if (yDelta != 0)
    {
      if (currentMove == MOVE_FALL_DOWN) frame = lookLeft ? keyFrames[0] + 5 : keyFrames[0] + 6;
      else frame = keyFrames[0] + (yAdjust + 5) % 5;
    }
    else
    {
      if (xDelta >= 0) frame = (xAdjust + 6) % 6; else frame = (6 - xAdjust) % 6;
      if (stage.getTileBehavior(xTile, yTile) == LodeRunnerStage.TILE_ROPE)
      {
        frame += keyFrames[0] + 7; if (lookLeft) frame += 6;
      }
      else 
      {
        ...
        if (lookLeft && frame == 0) frame = keyFrames[1];
        else if (lookLeft) frame += keyFrames[3];
        else frame += keyFrames[2];
      }
    }
    return frame;
  }
  ...
  public int getX()
  {
    if (xDelta >= 0) return (6 * xTile + xAdjust + (xAdjust + 6) % 2) * LodeRunnerStage.SPRITE_WIDTH[stage.spriteSize] / 6;
    else return (6 * xTile + xAdjust - (6 - xAdjust) % 2) * LodeRunnerStage.SPRITE_WIDTH[stage.spriteSize] / 6;
  }
  ...
  protected void setCurrentMove(int move)
  {
    ...
    {
      case MOVE_NONE:
        xDelta = 0; yDelta = 0; break;
      case MOVE_RUN_LEFT:
        lookLeft = true; xDelta = -1; yDelta = 0; break;
      case MOVE_RUN_RIGHT:
        lookLeft = false; xDelta = 1; yDelta = 0; break;
      case MOVE_CLIMB_UP:
        xDelta = 0; yDelta = -1; break;
  ...
  protected boolean shouldFall()
  {
    ...
    if (currentType == LodeRunnerStage.TILE_BRICK || currentType == LodeRunnerStage.TILE_LADDER || currentType == LodeRunnerStage.TILE_ROPE) return false;
    // Don't fall if standing on brick, or concrete, or at the top of a ladder
    int bottomType = stage.getTileBehavior(xTile, yTile + 1);
    if (bottomType == LodeRunnerStage.TILE_BRICK || bottomType == LodeRunnerStage.TILE_CONCRETE || bottomType == LodeRunnerStage.TILE_LADDER) return false;
    return true;
  }
  ...
  protected int getReverseMove()
  {
    switch (currentMove)
    {
      case MOVE_RUN_LEFT: return MOVE_RUN_RIGHT;
      case MOVE_RUN_RIGHT: return MOVE_RUN_LEFT;
      case MOVE_CLIMB_UP: return MOVE_CLIMB_DOWN;
      case MOVE_CLIMB_DOWN: return MOVE_CLIMB_UP;
  ...
  protected void computeNextMove()
  {
    ...
    if (!isPossibleMove(nextMove)) nextMove = MOVE_NONE;
    ...
    else if (nextMove == MOVE_CLIMB_DOWN)
    {
      ...
      int nextType = stage.getTileBehavior(xTile, yTile + 1);
      if (nextType != LodeRunnerStage.TILE_LADDER) nextMove = MOVE_FALL_DOWN;
    }
  }
  ...
  protected boolean computeNewPosition()
  {
    ...
    if (xNewAdjust < -3) { xNewAdjust += 6; xNewTile--; } else if (xNewAdjust > 3) { xNewAdjust -= 6; xNewTile++; }
    if (yNewAdjust < -2) { yNewAdjust += 5; yNewTile--; } else if (yNewAdjust > 2) { yNewAdjust -= 5; yNewTile++; }
    // Validate new position before applying
    if ((xNewTile == xTile && yNewTile == yTile) || canChangeTile(xNewTile, yNewTile))
    {
      xTile = xNewTile; yTile = yNewTile; xAdjust = xNewAdjust; yAdjust = yNewAdjust;
      return true;
    }
    else return false;
  }
  ...
  public void heartBeat()
  {
    ...
    if (stage.getTileBehavior(xTile, yTile) == LodeRunnerStage.TILE_BRICK) { kill(); return; }
    // Decrease (if any) the number of heartBeats before this character can move again
    if (delayBusy > 0) { delayBusy--; return; }
    // If the current move has ended, execute the next move
    if (xAdjust == 0 && yAdjust == 0) makeNextMove();
    // Compute new position
    computeNewPosition();
  }
