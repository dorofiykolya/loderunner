...
class LodeRunnerStage
{
  ...
  LodeRunnerStage(LodeRunnerCanvas canvas)
  {
    ...
    try
    {
      ...
      font = new GameFont("/Font.png", 3, 5, "0123456789/");
      for(int i=0;i<2;i++)
        sprites[i] = new GameSprite("/Tiles " + SPRITE_WIDTH[i] + "x" + SPRITE_HEIGHT[i] + ".png", SPRITE_WIDTH[i], SPRITE_HEIGHT[i], 0, 0);
  ...
  private class LoadingThread extends Thread
  {
    ...
    public void run()
    {
      try
      {
        ...
        DataInput input = new DataInputStream(getClass().getResourceAsStream("/LodeRunner.bin"));
        // Read level's buffer
        ...
        for (int i = 0; i < STAGE_WIDTH * STAGE_HEIGHT; i++)
        {
          ...
          if (backgroundTilesToRepaint != null) backgroundTilesToRepaint.addElement(new Integer(i));
          // Periodically yield to other threads
          if (i % STAGE_WIDTH == 0) yield();
        }
        // The show can go on...
  ...
  public void loadFromResource()
  {
    ...
    while (loadingThread != null) isLoaded = true;
    // Reset members
    isLoaded = false;
    hero = null; vilains = new Vector(); holes = new Vector();
    nChests = 0; exitEnabled = false; endHeroDied = false; endCompleted = false;
    if (backgroundTilesToRepaint != null) backgroundTilesToRepaint.removeAllElements();
  ...
  {
    if (xTile < 0 || xTile >= STAGE_WIDTH || yTile < 0 || yTile >= STAGE_HEIGHT) return TILE_OUTSIDE;
  ...
  public int getTileBehavior(int xTile, int yTile)
  {
    ...
    if (tile == TILE_CHEST || tile == TILE_MONK || tile == TILE_HERO ||tile == TILE_HOLE_EMPTY) tile = TILE_VOID;
    else if (tile == TILE_OUTSIDE || tile == TILE_HOLE_FULL) tile = TILE_CONCRETE;
    else if (tile == TILE_EXIT) tile = exitEnabled ? TILE_LADDER : TILE_VOID;
    return tile;
  }
  ...
  {
    int tile = getTile(xTile, yTile);
    if (tile == TILE_TRAP) tile = TILE_BRICK;
    else if (tile == TILE_MONK || tile == TILE_HERO || tile == TILE_OUTSIDE || tile == TILE_HOLE_FULL || tile == TILE_HOLE_EMPTY) tile = TILE_VOID;
    else if (tile == TILE_EXIT) tile = exitEnabled ? TILE_LADDER : TILE_VOID;
    return tile;
  }
  ...
  {
    if (xTile < 0 || xTile >= STAGE_WIDTH || yTile < 0 || yTile >= STAGE_HEIGHT) return;
  ...
  public void enableExit()
  {
    if (!exitEnabled)
    {
      ...
      if (backgroundTilesToRepaint != null)
        for (int i = 0; i < STAGE_WIDTH * STAGE_HEIGHT; i++)
          if (tiles[i] == TILE_EXIT) backgroundTilesToRepaint.addElement(new Integer(i));
    }
  }
  ...
  {
    if (spriteSize != SPRITE_NORMAL || backgroundImage == null) return;
  ...
  public void centerScreen(Graphics g)
  {
    ...
    if (isLoaded && hero != null)
    {
      ...
      if (w0 >= STAGE_WIDTH * SPRITE_WIDTH[spriteSize]) tx = (w0 - STAGE_WIDTH * SPRITE_WIDTH[spriteSize]) / 2;
      else if (hero.getCenterX() < w0 / 2) tx = 0;
      else if (hero.getCenterX() > STAGE_WIDTH * SPRITE_WIDTH[spriteSize] - w0 / 2) tx = w0 - STAGE_WIDTH * SPRITE_WIDTH[spriteSize];
      else tx = w0 / 2 - hero.getCenterX();
      if (h0 >= STAGE_HEIGHT * SPRITE_HEIGHT[spriteSize]) ty = (h0 - STAGE_HEIGHT * SPRITE_HEIGHT[spriteSize]) / 2;
      else if (hero.getCenterY() < h0 / 2) ty = 0;
      else if (hero.getCenterY() > STAGE_HEIGHT * SPRITE_HEIGHT[spriteSize] - h0 / 2) ty = h0 - STAGE_HEIGHT * SPRITE_HEIGHT[spriteSize];
      else ty = h0 / 2 - hero.getCenterY();
    }
    else
    {
      // Center stage
